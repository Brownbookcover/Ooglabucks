...
@onready var start_padding = _borehole.HEIGHT - _borehole.bottom_offset
...
func _ready() -> void:
	...
	for i in len(_borehole.layer_colors):
		_gradient_texture.gradient.set_color(i, _borehole.layer_colors[i])
	
	total_height = _borehole.layer_heights.reduce(func(a, n): return a + n)
	var depth = 0
...
func _process(_delta: float) -> void:
	...
	_depth_label.text = UI.MoneyConversionFunction(int(start_padding + _borehole.depth_px)) + "m"
